...
# Creates a public portfolio documentation page for the Grafana monitoring lab.
# Safe behavior:
# - reads screenshots only from docs/private/.../selected-portfolio
# - copies selected screenshots into a public docs/assets folder with clean names
# - creates/updates docs/operations/grafana-monitoring-lab.md
# - creates a timestamped backup if the Markdown file already exists
# - does not run git add or git commit automatically
#
# Important:
# Review all copied screenshots before committing them. Public screenshots must not contain
# passwords, tokens, private browser data, private tabs, personal addresses, or sensitive local paths.

$ProjectRoot = Get-Location

$PrivateScreenshotDir = Join-Path $ProjectRoot 'docs\private\screenshots\grafana-monitoring\selected-portfolio'
$PublicScreenshotDir = Join-Path $ProjectRoot 'docs\assets\screenshots\grafana-monitoring'
$DocDir = Join-Path $ProjectRoot 'docs\operations'
$DocPath = Join-Path $DocDir 'grafana-monitoring-lab.md'

if (-not (Test-Path $PrivateScreenshotDir)) {
    throw "Selected screenshot folder not found: $PrivateScreenshotDir"
}

New-Item -ItemType Directory -Force -Path $PublicScreenshotDir | Out-Null
New-Item -ItemType Directory -Force -Path $DocDir | Out-Null

$Mappings = @(
    @{
        Source = 'Screenshot 2026-06-11 223137.png'
        Target = '01-prometheus-target-health-up.png'
    },
    @{
        Source = 'Screenshot 2026-06-11 202625.png'
        Target = '02-reverse-proxy-healthy-https-ok.png'
    },
    @{
        Source = 'Screenshot 2026-06-11 182246.png'
        Target = '03-docker-stack-cadvisor-down.png'
    },
    @{
        Source = 'Screenshot 2026-06-11 182257.png'
        Target = '04-webhook-prometheus-target-down.png'
    },
    @{
        Source = 'Screenshot 2026-06-11 230134.png'
        Target = '05-antenna-dashboard-normal.png'
    },
    @{
        Source = 'Screenshot 2026-06-11 230208.png'
        Target = '06-antenna-dashboard-weak-signal.png'
    },
    @{
        Source = 'Screenshot 2026-06-11 231757.png'
        Target = '07-antenna-dashboard-many-errors.png'
    },
    @{
        Source = 'Screenshot 2026-06-12 133240.png'
        Target = '08-antenna-dashboard-no-packets-offline.png'
    },
    @{
        Source = 'Screenshot 2026-06-12 133352.png'
        Target = '09-webhook-antenna-weak-signal-alert.png'
    },
    @{
        Source = 'Screenshot 2026-06-12 152401.png'
        Target = '10-antenna-site-map-geomap.png'
    }
)

Write-Host "Copying selected screenshots to public docs assets..."
foreach ($item in $Mappings) {
    $SourcePath = Join-Path $PrivateScreenshotDir $item.Source
    $TargetPath = Join-Path $PublicScreenshotDir $item.Target

    if (-not (Test-Path $SourcePath)) {
        throw "Missing selected screenshot: $SourcePath"
    }
...
if (Test-Path $DocPath) {
    ...
    $BackupPath = "$DocPath.backup-$Timestamp"
    Copy-Item -Path $DocPath -Destination $BackupPath -Force
    Write-Host "Existing Markdown file backed up:"
...
Set-Content -Path $DocPath -Value $Markdown -Encoding UTF8

Write-Host ""
Write-Host "OK: Portfolio documentation prepared."
Write-Host "Markdown file:"
Write-Host "  $DocPath"
Write-Host "Public screenshot folder:"
Write-Host "  $PublicScreenshotDir"
Write-Host ""
Write-Host "Next checks:"
Write-Host "  git status --short"
Write-Host "  git diff --stat"
Write-Host ""
Write-Host "Important: Review copied public screenshots before committing."
